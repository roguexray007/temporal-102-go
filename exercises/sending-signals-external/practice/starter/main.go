...
import (
	...
	"log/slog"
	"os"
	pizza "temporal-102-go/exercises/sending-signals-external/practice"
...
func main() {
	...
	log.Println("Started pizzaworkflow", "WorkflowID", we.GetID(), "RunID", we.GetRunID())
	...
	fwe, err := c.ExecuteWorkflow(context.Background(), signalFulfilledOptions, pizza.FulfillOrderWorkflow, order)
	if err != nil {
		log.Fatalln("Unable to execute workflow", err)
	}
	log.Println("Started fulfill order workflow", "WorkflowID", fwe.GetID(), "RunID", fwe.GetRunID())
